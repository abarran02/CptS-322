...
class Meal(models.Model):
    ...
    TotalCalories = models.ManyToManyField(Food)
    
    def __str__(self) -> str:
        return self.MealType

    def addFoodItem(self, newDescription:str, newMeal:str, newCalories:int):
        newFoodModel = Food()
        newFoodModel.description = newDescription
        newFoodModel.meal = newMeal
        newFoodModel.calories = newCalories

    #def findTotalCalories(self):

class Food(models.Model):
    description = models.TextField()
    meal = models.CharField(max_length=64)
    calories = models.IntegerField()
    foodID = models.ForeignKey(Meal) # links to a certain meal ID
